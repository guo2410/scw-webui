...
import com.atguigu.scw.uiweb.config.AlipayConfig;
import com.atguigu.scw.uiweb.service.TOrderServiceFeign;
import com.atguigu.scw.uiweb.vo.req.OrderFormInfoSubmitVo;
...
public class TOrderController {
	...
	public String payOrder(OrderFormInfoSubmitVo vo, HttpSession session) {
		...
		// 1,下单
		log.debug("提交订单-立即支付OrderFormInfoSubmitVo={}", vo);

		OrderInfoSubmitVo orderInfoSubmitVo = new OrderInfoSubmitVo();

		BeanUtils.copyProperties(vo, orderInfoSubmitVo);

		UserRespVo userRespVo = (UserRespVo) session.getAttribute("LoginMenber");
